...
by the most occuring value in the particular column. For example, if
...
We do this seperately for the two parties.
```{r missing}
...
```{r TrainNB, cache=TRUE, include=FALSE}
...
```{r plotNBTESTloss, include=FALSE}
plot(c(0, 220), c(0, 0.6), type = 'n', 
xlab = "Size of training set", ylab = "mean 0/1-loss",)
colors = c("black", "blue", "red")
for(i in 1:3) { # 1 time NB
...
```{r LRtrainLoss, cache=TRUE, include=FALSE}
lr <- glm(party ~., data = Train, family = binomial("logit"))
...
```{r lg2robs, cache=TRUE, include=FALSE}
...
The error on the test set equals `r mean(bestCharPred != Test[,1])`, an slight improvement
over the performance of non-penalized logistic regression. 
Next, different sample sizes. From only $n = 4$, we have more ore less reached 
the 'asymptotic' error rate! Hence, penalized logistic regression does not only 
provide the best predictions, it does this also much, much faster than 
Naive Bayes. This indicates that this method should be preferred
also for small sample sizes. 
```{r penLR3, cache=TRUE, warning=FALSE}
tryGetPenalizedLoss <- function(x, y) {
  penalizedLR <-  glmnet(x, y, family = "binomial")
  cvPenalizedLR<-cv.glmnet(x, y, family="binomial", nfolds=5)
  pred  <- predict(penalizedLR, xTest)
  bestpred <- pred[ ,which(cvPenalizedLR$lambda==cvPenalizedLR$lambda.min)]
  bestCharPred <- rep("democrat", length(bestpred))
  bestCharPred[bestpred > 0] <- "republican"
...
y <- Train[,1]
xTest <-  sapply(Test[, -1], as.numeric) 
lossLR <- getPenalizedLRloss(x, y)
plot(lossLR[lossLR < 1], xlim = c(0, 220), ylim = c(0, 1), xlab = "size of training set", ylab = "mean 0/1-loss", type = 'l')
```
