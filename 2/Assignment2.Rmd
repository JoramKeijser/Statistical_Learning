...
erradic. This may be caused by tuning the penalization constant different for 
each sample size. 
The real advantage of penalizing shows when looking at the performance for different
sample sizes. From only $n = 4$, the smallest sample size for which the regression algorithm
is not hindered by linear dependencies in the data, we are very close to
...
getPenalizedLRloss <- function(x, y) {
  lossLR <- rep(1, length(sampleSizes))
  for(n in sampleSizes) { 
    xTemp <- x[1:n, ]
    yTemp <- y[1:n]
    lossLR[n] <- tryCatch(tryGetPenalizedLoss(xTemp, yTemp), error = function(c) 1)
  ...
  return(cbind(getNBloss(Train, Test), 
               LRloss = getLRloss(Train, Test), 
               LR2loss = getLRloss(Train[, 1:3], Test[1:3]),
               penLRloss = getPenalizedLRloss(x, y)))
}#getLosses
```

 
```{r Testcomparison2, warning=FALSE, cache=TRUE}
alpha = 1
TestPerformance <- calculateMeanPerformance()
```
