...
for(n in 1:218) { 
  tempTrain <- Train[1:n, ]
  lossLR[n] <- tryCatch(getLoss(tempTrain), error = function(c) 1)
}
...
lossLR2 <- rep(1, length(dim(Train)[1]))
for(n in 1:218) { 
  tempTrain <- Train[1:n, 1:3]
  lossLR2[n] <- tryCatch(getLoss(tempTrain), error = function(c) 1)
}
...
### 4. Comparison
```{r comparison}
performance = cbind(lossNB, lossLR, lossLR2)
```
### 5. Subset search.
