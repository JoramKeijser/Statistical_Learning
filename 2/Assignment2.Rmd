...
Physician. The majority, `r 100*mean(Data[DEM, "physician"] == "y")`, percent
of the democrats voted in favour. On the other hand, `r 100*mean(Data[REP, "physician"] == "y")` of the republicans voted against the proposal. This indicates that it should be possible
...
```
Mean 0/1 is the same when using all training data, namely
`r lossNB[length(lossNB), 1]`.

Comparable results for $n=2$ and $n=218$! Why? Maybe highly predicting
features. Try without physician etc. 

```{r plotNBloss}
plot(lossNB[, 1], type = 'l')
```
